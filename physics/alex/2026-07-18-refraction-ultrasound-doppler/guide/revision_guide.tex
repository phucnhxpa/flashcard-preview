...
\begin{center}
\begin{tikzpicture}[>=Latex,scale=.72,every node/.style={font=\small}]
% Keep the two panels distinct: labels sit beside/above arrows rather than
% sharing the caption baseline.
\node[font=\bfseries] at (-3.25,2.05){Transverse (light)};
\draw[->] (-5.7,0)--(-.9,0) node[right,font=\scriptsize]{energy};
\draw[domain=-5.7:-1,samples=100,thick] plot (\x,{.65*sin(180*(\x+5.7))});
\draw[<->,thick] (-5.25,.95)--(-5.25,-.95);
\node[anchor=east,font=\scriptsize] at (-5.4,-.72){oscillation};
\node[font=\scriptsize] at (-3.25,-1.35){oscillation $\perp$ energy transfer};

\node[font=\bfseries] at (3.45,2.05){Longitudinal (sound)};
\draw[->] (.85,0)--(5.95,0) node[right,font=\scriptsize]{energy};
\foreach \x in {1.15,1.3,1.45,2.2,2.95,3.1,3.25,4,4.75,4.9,5.05,5.8}{\draw (\x,-.55)--(\x,.55);}
\draw[<->,thick] (1.9,1)--(4,1) node[midway,above,font=\scriptsize]{oscillation};
\node[font=\scriptsize] at (1.3,-.82){C};
\node[font=\scriptsize] at (2.2,-.82){R};
\node[font=\scriptsize] at (3.1,-.82){C};
\node[font=\scriptsize] at (4,-.82){R};
\node[font=\scriptsize] at (3.45,-1.35){oscillation $\parallel$ energy transfer};
\end{tikzpicture}
\end{center}
