...
\hypersetup{
  pdftitle={Thermal Physics: Infrared, Conduction, Convection, Insulation, Specific Heat Capacity, Latent Heat and Gas Pressure},
  pdfauthor={Phuc Nguyen},
  pdfsubject={Edexcel IAL Physics --- Unit 5 Thermal Physics revision guide (lessons 9--10 July 2026)}
}
...
\begin{keybox}[Spec points 125--132 --- the exact official Edexcel IAL thermodynamics mapping]
Reproduced from the official specification (Topic~5.3 Thermodynamics,
\textit{official\_spec.txt} lines 1436--1448). Candidates must be able to:
\begin{itemize}[leftmargin=1.7em,itemsep=1pt]
  \item \textbf{125} --- use the equations $\dE = mc\,\dtheta$ \emph{and} $\dE = L\,\Delta m$.
  \item \textbf{126} --- \textbf{Core Practical 12:} calibrate a thermistor in a
        potential-divider circuit as a thermostat.
  \item \textbf{127} --- \textbf{Core Practical 13:} determine the specific latent heat of
        a phase change.
  \item \textbf{128} --- understand internal energy as the random distribution of
        \emph{potential and kinetic} energy amongst molecules.
  \item \textbf{129} --- understand absolute zero, and how the \emph{average kinetic
        energy} of molecules relates to the absolute temperature.
  \item \textbf{130} --- use the ideal-gas equation $pV = NkT$.
  \item \textbf{131} --- \textbf{Core Practical 14:} investigate the relationship between
        pressure and volume of a gas at fixed temperature.
  \item \textbf{132} --- be able to \emph{derive and use} $\tfrac{1}{2}m\langle c^2\rangle
        = \tfrac{3}{2}kT$.
\end{itemize}
\end{keybox}

\begin{plainbox}
\textbf{What these two lessons actually covered against the numbered points.} On the exact
lesson evidence, Alex explicitly taught only the \textbf{specific-heat-capacity half of
point 125}: he defined SHC, gave the equation $\dE = mc\,\dtheta$ with its units, outlined
an aluminium SHC method, and worked SHC calculations (the copper and avocado questions). He
also had Phuc reach the \textbf{definition} of specific latent heat --- energy to change
state, ``specific'' meaning \emph{per kilogram} --- but he did \emph{not} state or use the
equation $\dE = L\,\Delta m$ and did \emph{not} work any latent-heat number in these
lessons. Everything else in the numbered list below --- the $\dE = L\,\Delta m$
equation and latent-heat calculations, the formal \textbf{point-128} definition of internal
energy, all three core practicals (\textbf{CP12--CP14}), and the full \textbf{point-132}
derivation --- is included here as \emph{official-spec / reference expansion}, correct and
exam-relevant, but \emph{not} claimed as something Alex taught in these two sessions. The
kinetic picture of gas pressure Alex built (fixed-volume warm-up) is the qualitative
groundwork under \textbf{points 130 and 132}, not a formal treatment of them. The items
below are summarised so you know what still needs separate study:
\begin{itemize}[leftmargin=1.6em,itemsep=1pt]
  \item \textbf{CP12 (126) --- thermistor thermostat.} A thermistor forms one arm of a
        potential divider; as temperature rises its resistance falls, so the output voltage
        across the fixed resistor changes. Record output voltage against temperature to
        \emph{calibrate} the divider, so a chosen voltage can switch a heater/thermostat at
        a set temperature. \emph{Not taught in these lessons.}
  \item \textbf{CP13 (127) --- specific latent heat.} Supply measured electrical energy
        ($IV\,\Delta t$) to melt/boil a known mass; use a control to subtract energy gained
        from the surroundings; then $L = \dE / \Delta m$. \emph{Not taught in these
        lessons}; Alex taught only the \emph{definition} of specific latent heat
        (energy to change state, per kilogram), not the equation $\dE = L\,\Delta m$ or any
        latent-heat calculation.
  \item \textbf{CP14 (131) --- pressure vs volume at fixed temperature.} Trap a gas column,
        vary the pressure, read the volume at constant temperature; a graph of $p$ against
        $1/V$ is a straight line through the origin, confirming Boyle's law $pV=\text{const}$.
        \emph{Not taught in these lessons} (Boyle's law was mentioned only as background).
\end{itemize}
\end{plainbox}

\begin{plainbox}
\textbf{Where the qualitative content fits.} The infrared, conduction, convection and
insulation material Alex taught is \emph{not} itself among numbered points 125--132: it is
GCSE/ESAT foundation and synoptic ``controlling heat transfer'' context that Unit~5 assumes
and that the ESAT admissions test rewards. It is examined mainly through worded explanation
questions. Treat it as essential groundwork, not as a numbered Unit~5 requirement.
\end{plainbox}
...
\textbf{Thermal equilibrium:} the state in which there is \emph{no net transfer of thermal
energy} between two bodies (or between a body and its surroundings); when they are in
thermal contact this means they are at the \emph{same temperature}. Equal radiation emitted
$=$ radiation absorbed is the special \emph{radiative} case (used in the cooling-curve and
greenhouse arguments); it is not sufficient on its own when conduction, convection, work or
internal power generation are also present.
...
\]
The video/audio analysis records Phuc first offering "mass $\div$ volume" (that is
\emph{density}), with Alex correcting it to force per area. \emph{This particular slip is
secondary video-analysis evidence only} --- the exact transcript does not preserve it (it
records Phuc reaching "stronger force" and Alex restating $p=F/A$), so treat it as
medium-confidence context. Either way, keep density ($\rho = m/V$) and pressure ($p = F/A$)
firmly apart.
...
\emph{steady-state energy balance}, and it is the anchor for the greenhouse-effect and
cooling-curve arguments later. (Reserve the term \emph{thermal equilibrium} for the
stricter case of no net thermal transfer between bodies at equal temperature in thermal
contact; a body with energy flowing through it, or generating its own power, can hold a
constant temperature without being in thermal equilibrium.)
...
\begin{itemize}[leftmargin=1.3em,itemsep=1pt]
  ...
  \item Gave "mass $\div$ volume" for pressure $\to$ pressure is \emph{force per area}
        (\emph{video-analysis-only evidence}; not in the exact transcript --- see
        Section~\ref{sec:fixedgas-warmup} note above).
\end{itemize}
\end{watchbox}
...
\item[\ts{13:06--16:19} Herschel prism experiment.] White light through a prism;
  thermometers in each colour, with one left \emph{beyond} the red. \textbf{Phuc predicted
  the \emph{red} thermometer would be hottest}; Alex corrected him --- the thermometer
  \emph{beyond red} heats most $\Rightarrow$ \textbf{infrared} (invisible light that warms
  objects). Phuc asked Alex to repeat the experiment, and Alex redrew/re-explained it.
  Second correction (\ts{15:42--16:19}): infrared is specifically \emph{just beyond red},
  not "anything invisible."
...
\item[\ts{32:20--35:03} Cooling-curve experiment.] Black vs silver cans, same start
  ...
  gets hotter.
\item[\ts{35:15--35:36} Phuc's laser question.] Phuc asks whether transferred energy can be
  sent "at the laser." Alex answers yes in broad terms but \emph{defers} a proper treatment
  to the later waves/light/uses-of-radiation topic.
\item[\ts{35:41--45:41} Conduction.] Solid particle model; two-block melting-ice demo
  ...
  Asked why metals conduct best, \textbf{Phuc first said they "simply vibrate more"}; Alex
  redirected him to \textbf{free electrons}, which Phuc then supplied.
...
\item[\ts{03:08--10:28} Retrieval of the three mechanisms (diagnostic).] Phuc readily
  recalled \textbf{infrared radiation}, but \textbf{struggled to identify conduction} from
  the "collision of particles" clue (Alex had to supply it and note it "is not another form
  of radiation"), and \textbf{had forgotten convection}. His first convection attempt
  --- heated fluid "expands" and "merges," with rising collision "frequency" causing
  expansion --- was rebuilt by Alex into the correct chain: particles \emph{move faster}
  $\to$ \emph{spread apart} $\to$ \emph{less dense} $\to$ \emph{rise}. Alex noted convection
  "needs several repetitions." Exam tip: say the \textbf{right keywords in the right order}.
...
\item[\ts{25:53--33:33} Why trapped air also stops convection; real products.] Asked why
  trapped air also cuts \emph{convection}, \textbf{Phuc explained only the conduction part}
  (fewer particles), became confused, and suggested the heated trapped air might expand and
  \textbf{"drop down."} Alex corrected this: free air could circulate, but air divided into
  \emph{small immobilised pockets cannot flow}, so no convection current forms. Loft rolls
  ($\sim$\SI{10}{cm} $+$ \SI{25}{cm} $=$ \SI{35}{cm}); rigid foil-faced boards; triple
  glazing. A note that the best board is not always used, "for a reason we'll look at
  tomorrow."
\item[\ts{35:04--38:28} Into calculations: SHC.] "Layout" is the most important part of
  calculation questions. Retrieval of \emph{specific latent heat}: Phuc first offered "energy
  to transfer some heat," was steered to \textbf{energy needed to change state}, then guessed
  "specific" meant per square metre/cubic metre before reaching \textbf{per kilogram}
  (\ts{35:40--36:35}). Between this and the SHC definition, Alex posed an \emph{open}
  question --- which transfer mechanism matters most for global warming, with no single right
  answer --- and Phuc answered "solar panel," read as radiation (\ts{36:35--37:08}). SHC then
  defined; $\dE = mc\,\dtheta$; units derived as \si{\joule\per\kilogram\per\kelvin}.
...
\begin{watchbox}
\textbf{The full emit/absorb/reflect diagnostic (9 July \ts{23:44--26:38}).} This was not a
single slip but a sequence of separate corrections:
\begin{enumerate}[leftmargin=1.5em,itemsep=1pt]
  \item Phuc asked whether "absorbing" means keeping the energy. Alex fixed the vocabulary:
        \emph{emit $=$ give off}; \emph{absorb $=$ take in}.
  \item Phuc then said the surface \emph{best at reflecting is also best at absorbing}.
        \textbf{Wrong} --- for incoming radiation, \emph{reflection is the opposite of
        absorption}, so the best reflector is the \emph{worst} absorber.
  \item Phuc asked whether the \emph{emitter} is similar to the \emph{reflector}. Alex said
        \textbf{no}: the shiny surface is the \emph{worst} emitter/absorber (best reflector),
        while black/matt is the \emph{best} emitter/absorber (worst reflector).
\end{enumerate}
Alex used a "black surface with holes lets IR through, hard shiny surface bounces it" mental
model but explicitly warned it is \emph{not} the literal microscopic reality --- just a memory
aid.
\end{watchbox}
...
type of nuclear radiation." \textbf{Wrong.} "Radiation" broadly just means energy is given
off. \emph{Nuclear} radiation is specifically \textbf{alpha, beta, gamma} from the
\emph{nucleus}. Infrared and visible light are \textbf{electromagnetic radiation (photons)}.
Ordinary thermal infrared from a warm object is produced by the \emph{thermal motion of its
charged particles} --- vibrating/rotating molecules and lattice (bond) vibrations --- and
forms a broad continuous spectrum; it does \emph{not} come from the nucleus. (Alex did
mention, as an aside beyond GCSE, that a photon can also be released when an atomic electron
drops between energy levels; that produces line spectra and is a \emph{different} process
from the broad thermal-IR glow of everyday warm objects.)
\end{watchbox}
...
\begin{watchbox}
\textbf{Correction to Phuc (9 July \ts{42:47--45:00}).} Asked \emph{why} metals conduct
heat especially well, Phuc first said they "simply vibrate more." Alex redirected him to the
\textbf{free electrons}; Phuc then correctly answered that metals have many free electrons.
Vibration-passing happens in \emph{all} solids --- the free electrons are what make metals
\emph{special}.
\end{watchbox}
...
\begin{watchbox}
\textbf{Two convection corrections to Phuc (9 July).} (i) Cooling air does \emph{not} have to
turn to liquid to sink --- a gas can just become denser and sink while staying a gas. (ii)
In the dyed-water loop, Phuc asked whether it might \emph{"become gas"} and later
\emph{"blend"} with the rest; corrected --- the water is \emph{not} heated enough to change
state; it stays liquid and the dye simply reveals the circulating current. (The separate
"drop down" confusion arose later, on 10 July, for \emph{trapped air pockets}.)
\end{watchbox}
...
Double / triple glazing & Conduction (\emph{and} convection; radiation with a low-e
coating) & A poor-conducting gas (air/argon) fills the gap; a suitably \emph{narrow} gap
also suppresses convection; a low-emissivity coating cuts radiative loss. \\
...
% boiling plateau
\addplot[thick] coordinates {(6,4.3)(8.6,4.3)};
% gas (steam) warming
\addplot[thick] coordinates {(8.6,4.3)(9.7,5.2)};
...
\node[anchor=south] at (axis cs:7.3,4.3) {\scriptsize boiling ($L_v$)};
\node[anchor=south,rotate=38] at (axis cs:9.1,4.6) {\scriptsize gas (steam)};
\draw[dashed] (axis cs:2,0)--(axis cs:2,2.5);
\draw[dashed] (axis cs:6,0)--(axis cs:6,4.3);
\end{axis}
\end{tikzpicture}
\end{center}
The \textbf{sloped} sections are $\dE = mc\,\dtheta$ (temperature rising, one state ---
solid, then liquid, then \emph{gas/steam warming} after boiling completes). The
\textbf{flat plateaus} are $\dE = L\,\Delta m$ (state changing, temperature constant). For
the \emph{same mass} of substance and with energy (or, at constant power, time) on the
horizontal axis, the boiling plateau is much longer than the melting plateau because
$L_v \gg L_f$; that comparison only holds under those equal-mass / constant-power
conditions.
...
\subsubsection{Kinetic theory: deriving \texorpdfstring{$\tfrac{1}{2}m\langle c^2\rangle = \tfrac{3}{2}kT$}{½m<c²> = 3/2 kT} (spec point 132)}

Point 132 requires you to \emph{derive}, not just quote, the link between mean molecular
kinetic energy and temperature. Alex did not derive this in these lessons, so learn it here.

\begin{keybox}[Assumptions of the ideal-gas kinetic model]
\begin{itemize}[leftmargin=1.4em,itemsep=0pt]
  \item A gas contains a very large number $N$ of identical molecules in rapid, random
        motion.
  \item Molecules have negligible volume compared with the container.
  \item Collisions (with each other and the walls) are perfectly elastic; between
        collisions there are no intermolecular forces.
  \item The duration of a collision is negligible compared with the time between collisions.
\end{itemize}
\end{keybox}

Consider $N$ molecules of mass $m$ in a cube of side $L$ (volume $V=L^3$). Take one molecule
with velocity component $c_x$ towards the wall perpendicular to the $x$-axis.

\smallskip
\textbf{Step 1 --- momentum change in one wall collision.} The collision is elastic, so the
molecule rebounds with its $x$-velocity reversed, from $+c_x$ to $-c_x$. The change in the
molecule's momentum is therefore
\[
\Delta p_x = m c_x - (-m c_x) = 2 m c_x .
\]

\textbf{Step 2 --- time between successive impacts on the same wall.} Between two hits on
that wall the molecule must travel to the opposite wall and back, a distance $2L$, at speed
$c_x$:
\[
\Delta t = \frac{2L}{c_x}.
\]

\textbf{Step 3 --- mean force from one molecule on that wall.} By Newton's second law the
average force is the rate of change of momentum:
\[
F_x = \frac{\Delta p_x}{\Delta t} = \frac{2 m c_x}{2L / c_x} = \frac{m c_x^{2}}{L}.
\]

\textbf{Step 4 --- sum over all $N$ molecules.} Adding the contributions of every molecule
gives the total force on that wall, which we write using the \emph{mean} of the squared
$x$-components, $\langle c_x^2\rangle$:
\[
F = \frac{m}{L}\sum_{i=1}^{N} c_{x,i}^{2} = \frac{N m}{L}\,\langle c_x^{2}\rangle .
\]

\textbf{Step 5 --- pressure on the wall.} Divide the force by the wall area $A = L^2$:
\[
p = \frac{F}{L^{2}} = \frac{N m}{L^{3}}\,\langle c_x^{2}\rangle
  = \frac{N m}{V}\,\langle c_x^{2}\rangle .
\]

\textbf{Step 6 --- isotropy.} The motion is random, so no direction is special:
$\langle c_x^2\rangle = \langle c_y^2\rangle = \langle c_z^2\rangle$. Since
$c^2 = c_x^2 + c_y^2 + c_z^2$, taking the mean gives $\langle c^2\rangle = 3\langle
c_x^2\rangle$, i.e. $\langle c_x^2\rangle = \tfrac13\langle c^2\rangle$. Substituting,
\[
p = \frac{N m}{V}\cdot\tfrac13\langle c^2\rangle
\qquad\Longrightarrow\qquad
\boxed{\;pV = \tfrac{1}{3}N m \langle c^2\rangle\;}
\]
where $\langle c^2\rangle$ is the \textbf{mean-square speed}. Rewrite it exposing the mean
kinetic energy $\tfrac12 m\langle c^2\rangle$:
\[
pV = \tfrac{1}{3}N m \langle c^2\rangle = \tfrac{2}{3}N\left(\tfrac{1}{2}m\langle c^2\rangle\right).
\]
Now combine with the ideal-gas equation for $N$ molecules, $pV = NkT$ (spec point 130):
\[
\tfrac{2}{3}N\left(\tfrac{1}{2}m\langle c^2\rangle\right) = NkT
\qquad\Longrightarrow\qquad
\boxed{\;\tfrac{1}{2}m\langle c^2\rangle = \tfrac{3}{2}kT\;}.
\]

\begin{keybox}[Reading the result]
\begin{itemize}[leftmargin=1.4em,itemsep=1pt]
  \item The \emph{average translational kinetic energy} of a molecule is $\tfrac32 kT$: it
        depends only on the \textbf{absolute temperature} $T$ (in kelvin), and would be
        \emph{zero at \SI{0}{K}} --- the microscopic meaning of absolute zero (point 129).
  \item The \textbf{root-mean-square speed} is $c_{\mathrm{rms}} = \sqrt{\langle c^2\rangle}
        = \sqrt{\dfrac{3kT}{m}}$. It is \emph{not} the ordinary mean speed --- it is the
...
The engineering answer Alex built with Phuc: conduct the internal heat to a \textbf{large
radiator with high thermal-infrared emissivity} (large area, good emitter) that radiates
infrared to space; and give the sunward side a finish with \textbf{low solar absorptivity}
(reflective/white) so incoming solar radiation is mostly reflected rather than absorbed. A
radiator made \emph{large enough} can reject both the internally generated heat and the
absorbed fraction of the solar input.

\begin{plainbox}
\textbf{Balancing the power budget (units done properly).} At steady state the radiator
must \emph{reject} as much power as the craft \emph{gains}. Every term is a power in watts:
\[
P_{\text{reject}} \;=\; P_{\text{internal}} \;+\; \alpha\, G_{\text{solar}}\, A_{\text{projected}}
\;\;(-\ \text{any other outgoing terms}),
\]
where $P_{\text{internal}}$ is the electrical/heat power generated inside (W), $G_{\text{solar}}
\approx \SI{1361}{\watt\per\metre\squared}$ is the solar \emph{flux} near Earth,
$A_{\text{projected}}$ is the sunlit cross-sectional area (\si{\metre\squared}) and $\alpha$
is the dimensionless \emph{solar absorptivity}. The flux $G_{\text{solar}}$ must be
multiplied by an area (and by $\alpha$) before it can be added to a power --- you cannot add
\si{\watt\per\metre\squared} to watts directly. \emph{The lesson's spoken "$\sim 500$" had no
stated unit in the source, so no dimensional sum is performed here.} Lowering $\alpha$
(reflective/white sunward finish) shrinks the solar term; raising the radiator's
thermal-IR emissivity and area raises $P_{\text{reject}}$. This is why spacecraft and
astronauts carry reflective outer layers plus dedicated high-emissivity radiator panels.
\end{plainbox}
...
\caption{9 July, screenshot 02 --- completed opening retrieval with the fixed-volume gas
reasoning ($p\uparrow$ because faster, frequent wall collisions, force $\uparrow$). The
board shows only an \emph{incomplete} expression $p = F/\dots$ with no visible denominator;
the completed form $p=F/A$ is supplied by the transcript, not visibly written on this
screenshot.}
\label{fig:0109-02}
\end{figure}
...
\item \textbf{How does heat get from the Sun to Earth, and why is Earth heating up?} \\
  ...
  \emph{Correction:} infrared is electromagnetic radiation (photons) from the thermal
  motion of charged particles (molecular vibration/rotation and lattice vibrations), not
...
\item \textbf{Infrared-camera diagnostic: where is the water level?} (Spoken, 9 July
  \ts{21:42--23:35}.) Reading the thermal image of the can, Alex asked Phuc to infer the
  water fill level from the temperature pattern. \emph{Phuc first said "the top," then
  changed to "the middle"; Alex confirmed the middle} --- the lower part is warmer/brighter
  (water behind the wall), while the cooler upper region is air. \emph{Technique:} read a
  thermal image as a \emph{temperature map} --- the brighter/warmer band marks where the hot
  water actually is.
\end{enumerate}
...
\item \textbf{Which block melts the ice first?} (Both at \SI{21}{\celsius}, different
  ...
  \textbf{good conductor}. \emph{Phuc's two corrections here (9 July \ts{37:05--40:11}):}
  (i) he first said the supporting block "must be hot" --- corrected, since both blocks are
  at the same \SI{21}{\celsius}, so the difference is \emph{conductivity}; (ii) asked what
  this shows about block~2, he said it could "absorb and emit energy more" --- Alex reframed
  the correct conclusion as \emph{block~2 transfers energy quickly to the ice by contact}, so
  it is a \textbf{good thermal conductor} (this is conduction on contact, not the
  emit/absorb radiation vocabulary).
...
\item \textbf{Fill-ins: best insulator / good insulators.} \\
  ...
  best insulator --- corrected \emph{as a tutor's simplified chain}: within the lesson's
  particle picture, stronger bonds make a neighbour move \emph{with} the first particle
  (transmitting vibration), whereas weaker bonds transmit less. \textbf{Physics note:}
  ``weaker bonds $=$ better insulator'' is \emph{not} a safe general rule --- real thermal
  conductivity depends on lattice structure, phonon scattering, free electrons, density and
  defects. The reliable idea is that \emph{gases and vacua conduct poorly because particles
  are sparse or absent}; a solid's conductivity cannot be read off bond strength alone.
\end{enumerate}
...
\item \textbf{Dyed-water loop: what happens when one corner is heated?} \\
  ...
  convection current. \emph{Phuc's slips (9 July \ts{55:05--57:09}):} asked whether the
  liquid might \emph{"become gas,"} and later suggested heating could make it \emph{"blend"}
  with the rest of the water --- corrected: it is \emph{not} heated enough to change state;
  it stays liquid and the dye simply reveals the circulating current. \emph{(The separate
  "drop down" confusion belongs to the 10 July trapped-air discussion, not here --- see the
  house-insulation section.)}
...
\begin{watchbox}
\textbf{Trapped air stops convection too (10 July \ts{25:53--28:14}).} Asked why loft and
cavity insulation reduce \emph{convection} as well as conduction, Phuc explained only the
conduction side (fewer particles), then became confused and suggested the heated trapped air
might expand and \textbf{"drop down."} \emph{Correction:} trapped air's \emph{second} role is
that dividing it into \textbf{small immobilised pockets stops it circulating} --- free air
could rise and fall as a convection current, but pocketed air cannot flow, so no current
forms. (This "drop down" confusion is distinct from the 9 July dyed-water "become gas/blend"
slip.)
\end{watchbox}
...
\begin{enumerate}[leftmargin=1.6em,itemsep=1pt]
  ...
  \item Air just above the hot sand is heated by \textbf{conduction} from the ground. (In a
        \emph{gas}, conduction is energy transfer by \emph{molecular collisions / random
        molecular motion}, not the "passing vibrations" phrase used for a solid lattice.)
  \item That hot air becomes less dense and rises (local \textbf{convection}); the warm air
        mass is then carried the long distance toward Paris by \textbf{wind / large-scale
        advection}. Strictly, local buoyant \emph{convection} lifts the air, while the
        horizontal transport across a continent is \emph{advection} by prevailing winds ---
        the lesson bundled both under "convection."
...
\begin{plainbox}
\textbf{Prompt (exactly as printed on the slide).} "A copper block has a specific heat
capacity of \SI{3850}{\joule\per\kilogram\per\kelvin}. It has a mass of \SI{0.5}{\kilogram}
and is heated by \SI{3}{\kelvin}. How much energy is transferred to the copper block?"

\smallskip
\textbf{Solution using the printed value exactly as displayed.}
\[
\dE = mc\,\dtheta = 0.5 \times 3850 \times 3 = \SI{5775}{\joule} .
\]
The arithmetic on the printed data is correct. A strict significant-figure answer, with the
inputs \SI{0.5}{\kilogram} and \SI{3}{\kelvin} read to 1~s.f., would be
$\approx\SI{6e3}{\joule}$.

\smallskip
\textbf{Physics-correct solution.} The accepted specific heat capacity of copper is about
$c \approx \SI{385}{\joule\per\kilogram\per\kelvin}$ (the supplied textbook's figure of
$\sim\SI{3.9}{\kilo\joule}$ to heat \SI{1}{\kilogram} of copper by \SI{10}{\kelvin} implies
$\approx\SI{390}{\joule\per\kilogram\per\kelvin}$). With the physically correct value,
\[
\dE = 0.5 \times 385 \times 3 = \SI{577.5}{\joule}\quad(\approx\SI{6e2}{\joule}\ \text{to 1 s.f.}).
\]
\end{plainbox}
\begin{watchbox}
\textbf{Worksheet-typo flag (screenshot 05).} The printed value \SI{3850}{} is a
\textbf{factor-of-ten too large}: accepted copper SHC is $\approx\SI{385}{\joule\per\kilogram\per\kelvin}$,
so the \emph{printed slide} almost certainly gained an extra zero. The screenshot also shows
a handwritten "385" (which is the physically correct figure) and the target symbol started
as a $\theta$-like mark although the question asks for \emph{energy}, $E$. Because the
screenshot proves only the discrepancy --- not who wrote what or which mark came first --- do
\emph{not} claim the handwriting "dropped a zero." Report it honestly: the printed
\SI{3850}{} disagrees with accepted physics; quote the board exactly, then give the
physics-correct \SI{577.5}{\joule}. Always write the correct target symbol ($E$, not
$\theta$).
\end{watchbox}
...
c = \frac{\dE}{m\,\dtheta} = \frac{1294}{0.215 \times 2}
  = \frac{1294}{0.43} = 3009.302325\ldots\ \si{\joule\per\kilogram\per\kelvin}.
\]
\textbf{Report to a sensible precision.} Treating the printed \SI{2}{\kelvin} as an
exact/simple classroom value, the answer is limited by \SI{0.215}{\kilogram} (3~s.f.):
\[
\boxed{c \approx \SI{3.01e3}{\joule\per\kilogram\per\kelvin}\ (3~\text{s.f.})}.
\]
(If instead the \SI{2}{\kelvin} is read as measured data to 1~s.f., strict
least-significant-figure convention would give only $\SI{3e3}{\joule\per\kilogram\per\kelvin}$,
1~s.f.) The unrounded calculator value $3009.3\ldots$ is carried forward into Q4 below.
\end{plainbox}

\begin{watchbox}
\textbf{Unattributed board/tutor arithmetic error (not Phuc's).} The board shows a circled
"$= \SI{2500}{\joule\per\kilogram\per\kelvin}$," but that does \emph{not} follow from the
printed numbers. The exact transcript attributes the 2500 to \emph{Alex}, who explicitly
said "let's make up a number, $1300 \div 0.5$" and then "let's just call that 2,500" ---
and even that aside is internally wrong, since $1300 \div 0.5 = 2600$, not 2500. The
screenshot report finds \emph{no identifiable author} for the circled 2500, so it is
\textbf{not} logged as a Phuc mistake. Using the printed avocado data,
$1294 \div (0.215 \times 2) = 3009.3\ldots \approx \SI{3.01e3}{\joule\per\kilogram\per\kelvin}$,
\emph{not} 2500. \emph{Lesson:} always re-key the division; a circled answer --- even the
tutor's --- can still be wrong.
\end{watchbox}
...
\smallskip
\textbf{Solution.} Use the \emph{unrounded} $c$ from Q3 ($c = 3009.302325\ldots\,\si{\joule\per\kilogram\per\kelvin}$)
and rearrange $\dE = mc\,\dtheta$ for $\dtheta$:
\[
\dtheta = \frac{\dE}{mc} = \frac{3000}{0.215 \times 3009.302325\ldots}
  = 4.636785\ldots\ \si{\kelvin} \approx \boxed{\SI{4.64}{\kelvin}}\ (3~\text{s.f.}).
\]
\textbf{Precision convention.} The printed \SI{3000}{\joule} is written with no decimal point
or stated uncertainty, so it is significant-figure ambiguous. Here it is treated as an
\emph{exact/classroom-supplied} (equivalently 4-s.f.) value, so the answer is limited by
\SI{0.215}{\kilogram} (3~s.f.) and reported as \SI{4.64}{\kelvin}. \emph{If} \SI{3000}{\joule}
were read strictly as 1~s.f., least-significant-figure convention would give only
$\dtheta \approx \SI{5}{\kelvin}$ (1~s.f.). Either way the raw calculator value is
$4.636785\ldots\,\si{\kelvin}$.

\smallskip
\emph{(The board left Q4 unfinished at "$c=$"; this is the intended completion. Carry the
full calculator value of $c$ through, then round \emph{once} at the end. If instead you used
the board's circled $c=\SI{2500}{}$, you would get
$\dtheta = 3000/(0.215\times2500) = \SI{5.58}{\kelvin}$ --- a separate error-branch, and
another reason to reject the 2500 and fix Q3 first.)}
\end{plainbox}
...
\subsection{Further visible and spoken prompts (recorded for completeness)}
\label{sec:visible-prompts}
These prompts appeared on the board or were spoken aloud but were \emph{not} fully worked in
the lessons. They are logged here so the exercise record is complete; where no worked answer
is recoverable, that is stated rather than invented.

\begin{enumerate}[leftmargin=1.6em,itemsep=5pt,label=\textbf{F\arabic*.}]
\item \textbf{Herschel "which thermometer becomes hottest?" (spoken, 9 July \ts{13:06--16:19}).}
  Alex asked which thermometer --- placed in the split colours, with one left beyond the red
  --- would show the greatest temperature rise. \emph{Phuc predicted the \textbf{red}
  thermometer.} \emph{Answer:} the thermometer \emph{just beyond the red} rises most,
  revealing \textbf{infrared}. Alex repeated/redrew the setup at Phuc's request. (Worked
  fully in Section~\ref{sec:theory}.)

\item \textbf{Laser energy-transfer question (spoken, 9 July \ts{35:15--35:36}).} Phuc asked
  whether transferred energy can be sent "at the laser." \emph{Alex's response:} yes, in
  broad terms, but a proper treatment of light/lasers and the uses of different radiation was
  \textbf{deferred to the later waves topic}. \emph{No worked answer was given in this lesson}
  --- logged as a deferred question, not a completed exercise.

\item \textbf{"Balloon fire water demo?" (visible board note, 9 July, screenshot 06).} On the
  conduction demo slide the purple text \textit{"Balloon fire water demo?"} is visible ---
  the classic demonstration where a water-filled balloon resists bursting in a flame because
  the water conducts and stores energy. \emph{It appears only as a written prompt/demo idea;
  it was not carried out or worked through on the recording}, so no solution is reconstructed
  here. \emph{Visible prompt only.}

\item \textbf{Focus Practical --- beaker cover (visible board aim, 10 July, screenshot 01).}
  The slide "\textit{4 -- Focus Practical -- investigating energy transfers}" carries the aim
  \textbf{"to design (and to explain the design of) a cover for a beaker that will keep it
  warm for a long time,"} with a skills line: \textbf{method; safety; identify variables;
  communicate findings in a graph.}
  \begin{itemize}[leftmargin=1.4em,itemsep=1pt]
    \item \emph{Expected method:} hot water in a beaker, trial different covers (e.g.\ trapped
          air/insulating lid, reflective foil), record temperature against time, compare
          cooling curves.
    \item \emph{Variables:} independent $=$ cover type/material; dependent $=$ water
          temperature (or temperature drop); control $=$ starting temperature, water volume,
          beaker, room conditions, timing intervals.
    \item \emph{Safety:} hot water/scald risk --- handle with care, avoid spills.
    \item \emph{Communicate:} plot temperature--time cooling curves; the best cover gives the
          \emph{shallowest} curve. The design should attack all three mechanisms (trapped air
          for conduction/convection, a reflective inner face for radiation).
    \item \emph{Status:} the aim/skills were displayed as a practical brief; \emph{no fully
          worked student answer is recoverable} from the lesson, so the method above is the
          expected approach, marked as such.
  \end{itemize}

\item \textbf{Open discussion: which transfer mechanism matters most for global warming?}
  (Spoken, 10 July \ts{36:35--37:08}.) Alex asked which energy-transfer mechanism is most
  important in reducing global warming and \emph{explicitly said there is no single right or
  wrong answer}. \emph{Phuc answered "solar panel,"} which Alex interpreted as choosing
  \textbf{infrared/radiation}, and provisionally accepted before moving on. \emph{Logged as
  an open discussion prompt, not a single-answer exercise} --- a defensible choice with a
  clear mechanism-based justification is what earns credit here.
\end{enumerate}
...
Herschel: predicted the \emph{red} thermometer hottest & The thermometer \emph{just beyond
red} rises most --- that band is infrared & "Hottest" is beyond the visible red, not red
itself & R2 \\
...
Best reflector "is also best absorber" & Reflection is the \emph{opposite} of absorption;
best reflector $=$ worst absorber/emitter & Reflect $\ne$ absorb; emitter $\ne$ reflector &
R2 \\
...
R2 \\
Infrared is "weak nuclear radiation" & Infrared is EM radiation (photons) from thermal
charge motion / bond vibrations; nuclear $=$ $\alpha,\beta,\gamma$ from the nucleus &
Nuclear means \emph{from the nucleus} only & R3 \\
...
Metals conduct best "because they vibrate more" & All solids pass vibrations; metals are
special because of \textbf{free electrons} & For metals, name free electrons, not extra
vibration & R4 \\
"Strong bonds" make the best insulator & (Tutor's simplified chain) strong bonds
\emph{transmit} vibration; the reliable rule is that \emph{fewer/no particles} insulate &
Gases/vacua insulate best (sparse/no particles); don't judge a solid by bond strength alone
& R4 \\
Dyed-water loop "becomes gas / blends" when heated & Not heated enough to change state;
liquid circulates and the dye reveals the current & Gentle heating $\Rightarrow$ convection,
not a state change & R5 \\
...
Trapped air might "drop down" (so no convection) & Small \emph{immobilised pockets} cannot
circulate --- that is what stops convection & Pocketed air cannot flow $\Rightarrow$ no
current & R5 \\
...
Ice-block conductor "absorbs and emits energy more" & (10 July \ts{37:05--40:11}) The block
\emph{transfers energy quickly to the ice by contact} $=$ good \emph{conductor}; that is
conduction, not radiation emit/absorb & On contact, use conduction language, not
emit/absorb & R4 \\
Specific latent heat is "energy to transfer some heat" & (10 July \ts{35:40--36:35}) It is
the energy needed to \emph{change state} (no temperature change); then \emph{specific} $=$
per kilogram & Latent heat $=$ energy to change state, per kg & R6 \\
...
\begin{plainbox}
\textbf{Not on this list (deliberately).} The circled avocado
"$c=\SI{2500}{}$" is \emph{not} a Phuc misconception: the transcript attributes the spoken
2500 to Alex's own made-up aside and the screenshot has no identifiable author. It is
handled as an unattributed board/tutor arithmetic error in
Section~\ref{sec:exercises}, not as a personalised weakness here. The board's printed
copper $c=\SI{3850}{}$ is likewise a worksheet typo, not a Phuc error.
\end{plainbox}
...
\\
\textbf{Solution.} Infrared is \textbf{electromagnetic} radiation (photons). The thermal
infrared emitted by a warm object comes from the \emph{thermal motion of its charged
particles} --- vibrating/rotating molecules and lattice vibrations --- giving a broad
continuous spectrum. \emph{Nuclear} radiation is alpha, beta or gamma emitted \emph{from the
nucleus}. "Radiation" alone just means energy is given off, so always specify the type.
\end{keybox}
...
(better conductor). (b) The dependable insulator rule is \emph{fewer or no particles}:
trapped air (few particles) or a vacuum (none) transfer energy poorly. The lesson's
"weaker bonds insulate better" is only a simplified particle-chain aid --- a real solid's
conductivity also depends on lattice structure, free electrons and phonon scattering, so do
not use bond strength alone to rank solid insulators.
\end{keybox}
...
\textbf{Solution.}
\[
c = \frac{\dE}{m\,\dtheta} = \frac{1294}{0.215\times 2} = 3009.3\ldots
  \approx \SI{3.01e3}{\joule\per\kilogram\per\kelvin}\ (3~\text{s.f.}).
\]
\[
\dtheta = \frac{\dE}{mc} = \frac{3000}{0.215\times 3009.3\ldots} = 4.6368\ldots
  \approx \SI{4.64}{\kelvin}\ (3~\text{s.f.}).
\]
Here \SI{3000}{\joule} is taken as an exact/classroom-supplied (4-s.f.) value, so the answer
is limited to 3~s.f.\ by the mass; if \SI{3000}{\joule} were read strictly as 1~s.f., the
reported result would be \SI{5}{\kelvin}. Layout first, substitute, carry the
\emph{unrounded} $c$ into the next part, and \emph{re-key the division} to check --- a neatly
circled answer can still be arithmetically wrong.
\end{keybox}
...
internal heat to a \emph{large radiator with high thermal-infrared emissivity} (good
emitter, large area) that radiates infrared to space, and give the sunward side a
\emph{low-solar-absorptivity (reflective/white)} finish to reflect incoming solar radiation.
Colour alone is not enough: a matt-black surface emits well but also \emph{absorbs} sunlight
strongly, so real spacecraft use finishes chosen for high IR emissivity \emph{and} low solar
absorptivity. Size the radiator to reject both the internal power and the absorbed fraction
of the solar input (power budget $P_{\text{reject}} = P_{\text{internal}} + \alpha\,
G_{\text{solar}} A_{\text{projected}}$, all terms in watts).
\end{keybox}
...
\textbf{Solution.} Sloped sections (solid warming, liquid warming, then \emph{gas/steam
warming} once boiling is complete) obey $\dE = mc\,\dtheta$. Flat plateaus obey
$\dE = L\,\Delta m$: melting at the fusion plateau, boiling at the (much longer)
vaporisation plateau, because
$L_v(\SI{2.26e6}{\joule\per\kilogram}) \gg L_f(\SI{3.34e5}{\joule\per\kilogram})$ ---
provided you compare the \emph{same mass} of substance with energy (or, at constant heating
power, time) on the horizontal axis. Temperature is constant during a change of state
because energy goes into potential energy (breaking/loosening bonds), not kinetic energy.
\end{keybox}
...
\item A \SI{0.5}{\kilogram} copper block is heated by \SI{3}{\kelvin}. The board printed
      $c=\SI{3850}{\joule\per\kilogram\per\kelvin}$; the accepted value is
      $c\approx\SI{385}{\joule\per\kilogram\per\kelvin}$. Find the energy transferred using
      each, and say which is physically correct.
\end{enumerate}
...
\item Printed value: $\dE = 0.5\times3850\times3 = \SI{5775}{\joule}$. Physics-correct:
      $\dE = 0.5\times385\times3 = \SI{577.5}{\joule}$. The accepted copper value is
      $\approx\SI{385}{\joule\per\kilogram\per\kelvin}$, so \SI{577.5}{\joule}
      ($\approx\SI{6e2}{\joule}$) is physically correct; the printed \SI{3850}{} is a
      factor-of-ten worksheet typo.
\end{enumerate}
\end{plainbox}
...
\begin{itemize}[leftmargin=1.4em,itemsep=2pt]
  ...
  \item \textbf{Thermal equilibrium:} no net transfer of thermal energy between bodies
        (equal temperature when in thermal contact); emission rate $=$ absorption rate is
        the radiative special case.
