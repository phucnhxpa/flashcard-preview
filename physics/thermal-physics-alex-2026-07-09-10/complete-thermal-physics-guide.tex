...
\begin{plainbox}
\textbf{What these two lessons actually covered against the numbered points.} Alex's
teaching sat mainly on \textbf{point 125} (both equations, with worked SHC and latent-heat
calculations), touched \textbf{point 128} (internal energy as random kinetic $+$ potential
energy) and \textbf{point 129} (absolute zero as the minimum-motion limit), and built the
kinetic picture of gas pressure that underpins \textbf{points 130 and 132}. The lessons
did \emph{not} carry out or examine the three numbered core practicals, and did not
formally \emph{derive} point 132. Those are flagged below and summarised so you know what
still needs separate study:
\begin{itemize}[leftmargin=1.6em,itemsep=1pt]
  ...
  \item \textbf{CP13 (127) --- specific latent heat.} Supply measured electrical energy
        ...
        lessons} (the equation itself, point 125, was taught).
...
Consider $N$ molecules of mass $m$ in a cube of side $L$ (volume $V=L^3$). Tracking momentum
change per wall collision, summing over all molecules and all three directions, and using
that the motion is isotropic ($\langle c_x^2\rangle = \langle c_y^2\rangle = \langle
c_z^2\rangle = \tfrac13\langle c^2\rangle$), the standard kinetic-theory result for the
pressure on the walls is
\[
...
$\dtheta = 3000/(0.215\times2500) = \SI{5.58}{\kelvin}$ --- another reason to reject the
2500 and fix Q3 first.)}
\end{plainbox}
...
\]
Layout first, substitute, carry the \emph{unrounded} $c$ into the next part, and
\emph{re-key the division} to check --- a neatly circled answer can still be arithmetically
wrong.
\end{keybox}
