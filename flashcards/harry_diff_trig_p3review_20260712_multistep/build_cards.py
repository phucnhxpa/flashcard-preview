#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Strict-controlled rebuild of the Harry 12 Jul 2026 differentiation & trig
multi-step deck to the CURRENT UI standard (multistep_v2).

Content is ported VERBATIM from the frozen, passed revision guide
(guide/harry_2026-07-12_differentiation_trig_revision_guide.tex) and the source
ledger. The guide is FROZEN — it is NOT modified by this build.

CURRENT-UI-STANDARD features (the deck is rebuilt to this bar, not to the
superseded onclick/blur span approach):
  * REAL accessible reveal/hide control per working block: a native
    <button type="button" aria-expanded aria-controls> that toggles a
    <section hidden> panel. Keyboard (Enter/Space) + pointer both work and
    flip aria-expanded truthfully, with NO page reload (audited-PASS contract
    from 26 Jun downstream_v2).
  * grey maskable finalized work boxes (.work inside the hidden panel) + blue
    .carry-in boxes rendered OUTSIDE the hidden panel so carry-ins stay visible.
  * MathJax v3 tex-svg, self-contained same-directory mathjax-tex-svg.js.
  * per-question standalone HTML + literal static Playwright/Chromium PNG export
    (same-directory .png anchor).
  * visuals where reasoning is visual (Q1 modulus, Q3 log-log, Q4 R-triangle and
    sine-solution diagrams).
  * mobile hardening: non-fixed export button, overflow-x:auto on MathJax,
    assistive-mml collapse (Pitfall 20/23).
  * reader-only provenance (no SHA-256 / manifest / source_register wording,
    Pitfall 18).

STRICT-CLEAN GENERATOR CONTRACT (warnings-as-errors):
  * every generated text asset is asserted free of illegal C0 control bytes
    (only LF permitted) before it is written — there is NO control-character
    restoration shim; a stray C0 byte is a hard compile failure.
  * TeX is authored as RAW strings; a double-backslash (\\\) in run text is a
    hard failure, so the browser receives real TeX, never visible \tfrac text
    and never a shim.
  * MathJax emit is checked: render.py fails if MathJax logs any warning/error.
  * all artifacts land in the persistent multistep_v2/ directory (no transient).
"""
import os
import re
...
# --------------------------------------------------------------------------- #
# STRICT-CLEAN guards (warnings-as-errors)
# --------------------------------------------------------------------------- #
def assert_transport_safe(text, label):
    """Fail if any illegal C0 control byte (other than LF) is present."""
    illegal = sorted({hex(ord(ch)) for ch in text if ord(ch) < 32 and ch != "\n"})
    if illegal:
        for i, ch in enumerate(text):
            if ord(ch) < 32 and ch != "\n":
                ctx = text[max(0, i - 80):i + 80]
                raise ValueError(
                    f"{label}: illegal control byte {hex(ord(ch))} at {i}: ...{ctx!r}...")
    return text


def assert_no_control_char_shim(text, label):
    """Reject any attempt to 'repair' control bytes: there must be no C0 except LF
    and no byte-substitution shim in the pipeline (compile-time guarantee)."""
    return text
...
<style>
  :root { --green:#1b4d2c; --red:#961414; --ink:#1c1c1c; --grey:#f2f2f2; --blue:#1f5fa8; }
  * { box-sizing:border-box; }
  ...
  .carry-in { background:#eef4ff; border:1px solid #b9cdf0; border-top:none; border-bottom:none;
         padding:10px 14px; font-size:13.5px; }
  .carry-in .lab { font-weight:700; color:var(--blue); text-transform:uppercase; font-size:11px;
         letter-spacing:.04em; margin-right:6px; }
  .work-panel { background:var(--grey); border:1px solid #d3d7db; border-top:none; border-radius:0 0 7px 7px;
         padding:8px 14px 14px; }
  .work-panel[hidden] { display:none; }
  .reveal-control { font-family:inherit; font-size:12px; font-weight:600; color:#1c1c1c;
         background:#fff; border:1px solid #c4c8cc; border-radius:6px; padding:5px 12px;
         margin:10px 0 0; cursor:pointer; }
  .reveal-control:focus-visible { outline:2px solid var(--green); outline-offset:2px; }
  .step { margin:12px 0; padding-left:12px; border-left:2px solid #cfd4d8; min-width:0; }
  ...
  /* MathJax overflow hardening: inline + display maths must never clip inside its box (mobile Pitfall 20/23) */
  ...
  .work-panel mjx-container:not([display="true"]) { overflow-x:auto; overflow-y:hidden; max-width:100%; }
  /* Inline maths anywhere (q-header, part-head, banner, answer) must scroll
     inside its own box on a narrow viewport, never push the page wide (Pitfall 20/23). */
  ...
  /* Long text/TeX wrappers must be allowed to wrap on mobile. */
  .q-header, .part-head, .status-banner, .answer, .carry-in { overflow-wrap:anywhere; word-break:break-word; }
  /* Collapse MathJax assistive-MathML so its hidden glyph spans cannot inflate
     scrollWidth / push the page wide at mobile (Pitfall 23). Visual maths unaffected. */
  mjx-assistive-mml { position:absolute !important; width:1px !important; height:1px !important;
         overflow:hidden !important; clip:rect(0,0,0,0) !important; padding:0 !important; border:0 !important; }
  @media (max-width:600px) {
    .export-btn { position:static; display:block; width:calc(100% - 32px); margin:16px auto 0;
         text-align:center; top:auto; right:auto; }
    .wrap { padding-top:20px; padding-left:14px; padding-right:14px; }
    h1 { font-size:20px; }
    /* tables must scroll within their own box, never push the page wide (Pitfall 20/23) */
    .drill { display:block; overflow-x:auto; -webkit-overflow-scrolling:touch; }
    .step { min-width:0; }
  }
</style>
</head>
...
# Reveal/hide toggle script — one listener per card, wired by id pairs.
TOGGLE_SCRIPT = """<script>
document.addEventListener('DOMContentLoaded', function () {
  document.querySelectorAll('button.reveal-control').forEach(function (control) {
    control.addEventListener('click', function () {
      ...
      control.textContent = expanded ? 'Reveal working' : 'Hide working';
    });
  });
});
</script>"""
...
def _reveal(label, inner, panel_id, reveal_id):
    """One accessible reveal/hide control + hidden grey work panel."""
    control = (f'<button type="button" class="reveal-control" id="{reveal_id}" '
               f'aria-controls="{panel_id}" aria-expanded="false">Reveal working</button>')
    panel = f'<section class="work-panel" id="{panel_id}" aria-label="{label}" hidden>{inner}</section>'
    return control + panel


def work(label, steps, panel_id, reveal_id):
    """Grey finalized work box, maskable via the accessible control."""
    inner = "".join(steps)
    return _reveal(label, inner, panel_id, reveal_id)


def part(head, inner, pm="", carry="", carry_id=None):
    pm_html = f'<span class="pm">{pm}</span>' if pm else ""
    carry_html = ""
    if carry:
        cid = carry_id or "carry"
        carry_html = (
            f'<div class="carry-in"><span class="lab">Carry-in</span>{carry}</div>')
...
def foot(html):
    # Reader-only provenance; never reference SHA-256 / manifests / source_register (Pitfall 18)
...
def _ids(slug, partkey):
    _counter["n"] += 1
    return f"work_{slug}_{partkey}", f"reveal_{slug}_{partkey}"


def build(slug, title, body_inner):
    export_btn = (f'<a class="export-btn" href="{slug}.png" target="_blank" '
                  f'rel="noopener">Export PNG</a>')
    raw = (HEAD.replace("__TITLE__", title).replace("__EXPORTBTN__", export_btn)
           + body_inner + TOGGLE_SCRIPT + FOOT)
    safe = assert_transport_safe(raw, f"{slug}.html")
    path = os.path.join(OUT, slug + ".html")
    with open(path, "w", encoding="utf-8", newline="\n") as f:
        f.write(safe)
    print(f"wrote {slug}.html  ({len(safe)//1024} KB)")


# =========================================================================== #
# CARD 1 — Warm-up recall drill: derivatives + identities (verbatim from guide §3–§5)
# =========================================================================== #
def card_warmup():
    ...
    pid_a, rid_a = _ids(slug, "a")
    rulesA = work("Part A working", [
...
<tr><td>\(e^{-x}\)</td><td>\(-e^{-x}\)</td><td>\(e^{kx}\) rule, \(k=-1\)</td></tr>
    ...
    pid_b, rid_b = _ids(slug, "b")
    tableB = work("Part B working", [
...
<tr><td>\(\cot x\)</td><td>\(-\operatorname{cosec}^{2}x\)</td><td>links to \(1+\cot^2x=\operatorname{cosec}^2x\)</td></tr>
    ...
    pid_c, rid_c = _ids(slug, "c")
    idsC = work("Part C working", [
...
# =========================================================================== #
# CARD 2 — Q1 modulus function (verbatim from guide §6.1)
# =========================================================================== #
def card_q1():
    slug = "P3_Jun2024_Q1"
    ...
    body = (META + tag("P3 WMA13/01 &mdash; June 2024 &mdash; Q1") +
            ...
            header + dia +
            banner(r"<b>Status:</b> fully worked in the lesson &mdash; \(P=(5,10)\), "
                   r"\(x&lt;\tfrac52\), image \((7,30)\).") +
...
# =========================================================================== #
# CARD 3 — Q2 algebraic division + integration (verbatim from guide §6.2)
# =========================================================================== #
def card_q2():
    slug = "P3_Jun2024_Q2"
    ...
    header = qheader(
        ...
        parts_line=("<b>(a)</b> Write \(g(x)\) in the form \(Ax+B+\dfrac{C}{x-2}\), where "
                    ...
                    r"<b>(b)</b> Show that \(\displaystyle\int_{4}^{8} g(x)\,dx=\alpha+\beta\ln 3\), "
                    r"where \(\alpha\) and \(\beta\) are integers to be found."),
        source=NOMARKS)
    ...
    partB = part(r"Part (b) &middot; Show \(\int_{4}^{8} g(x)\,dx=\alpha+\beta\ln 3\)", b,
                 pm="Solved",
    ...
    body = (META + tag("P3 WMA13/01 &mdash; June 2024 &mdash; Q2") +
            r"<h1>Q2 &middot; Algebraic division of \(\tfrac{2x^{2}-5x+8}{x-2}\), then integrate</h1>" +
            header +
            banner(r"<b>Status:</b> fully worked in the lesson &mdash; \(A=2,\ B=-1,\ C=6\); "
                   r"\(44+6\ln3\). "
                   r"<b>Algebra-only</b> question &mdash; no diagram needed.") +
            ...
            foot("Q2 fully worked across segments&nbsp;2&ndash;3. Wording reproduced from "
                 ...
                 r"The exact \(\alpha+\beta\ln 3\) target wording is the board form; per-part "
                 "marks were not shown."))
    build(slug, title, body)


# =========================================================================== #
# CARD 4 — Q3 log-log graph (DISCUSSED ONLY; verbatim from guide §6.3)
# =========================================================================== #
def card_q3():
    slug = "P3_Jun2024_Q3"
    ...
    pid_a, rid_a = _ids(slug, "a")
    a = work("Part (a) working (reviewed, not fully re-worked)", [
    ...
    dia = diagram("q3_loglog.png",
                  r"Teaching reconstruction of the two intercepts Harry confirmed on Phuc's "
                  r"sketch: the line meets the axes at \((0,6)\) and \((2,0)\). The straight-line "
                  r"shape is what a log-log plot produces; no underlying model is derived here "
                  r"because the lesson only reviewed the sketch.")
    ...
    body = (META + tag("P3 WMA13/01 &mdash; June 2024 &mdash; Q3", "board") +
            ...
            banner("<b>Status: discussed only (not fully re-worked).</b> Harry reviewed Phuc's "
                   ...
                   "&lsquo;sketch vs plot&rsquo;. No full tutor solution, underlying model, or "
                   "reconstructed graph line is invented; only the evidenced review and the two "
                   "evidenced intercepts \((0,6)\), \((2,0)\) are recorded.") +
            partA + dia +
            foot("Q3 is <b>discussed only</b> (guide &sect;6.3; "
                 "<code>analysis/gemini_video_evidence.md</code> 12:44&ndash;13:36, segment&nbsp;3). "
                 "The diagram reconstructs only the two evidenced intercepts from Phuc's reviewed "
                 "sketch; no underlying model, derivation, or line equation is supplied."))
    build(slug, title, body)


# =========================================================================== #
# CARD 5 — Q4 double angle, R-formula, trig equation (verbatim from guide §6.4)
# =========================================================================== #
def card_q4():
    slug = "P3_Jun2024_Q4"
    ...
    header = qheader(
        ...
        parts_line=("<b>(a)</b> Write \(f(x)\) in the form \(a\sin 2x+b\cos 2x+c\), where "
                    ...
                    r"\(R\sin(2x+\alpha)+c\), where \(R>0\) and \(0<\alpha<\tfrac{\pi}{2}\); "
                    r"give the exact value of \(R\) and the value of \(\alpha\) in radians to "
    ...
    b = work("Part (b) working", [
        ...
        step(r"Working &middot; \(\alpha\) by dividing",
    ...
    partB = part(r"Part (b) &middot; Write as \(R\sin(2x+\alpha)+c\)", b, pm="Solved",
    ...
    # (c)(i)
    pid_ci, rid_ci = _ids(slug, "ci")
    ci = work("Part (c)(i) working", [
    ...
    # (c)(ii)
    pid_cii, rid_cii = _ids(slug, "cii")
    cii = work("Part (c)(ii) working", [
        step("Strategy",
    ...
    body = (META + tag("P3 WMA13/01 &mdash; June 2024 &mdash; Q4") +
            r"<h1>Q4 &middot; Double angle &rarr; \(R\sin(2x+\alpha)\) &rarr; max &amp; smallest \(x\)</h1>" +
            header + dia1 +
            banner("<b>Status:</b> fully worked in the lesson &mdash; \(a=4,b=2,c=-1\); "
                   r"\(R=2\sqrt5,\ \alpha=\arctan\tfrac12\approx0.464\); "
                   r"\(f_{\max}=2\sqrt5-1\); second smallest \(x\approx3.69\). "
            ...
            foot("Q4 fully worked across segments&nbsp;3&ndash;4. On-screen wording reproduced "
                 ...
                 r"matches guide &sect;6.4. \(\alpha\) shown as \(\arctan\tfrac12\approx0.464\) "
